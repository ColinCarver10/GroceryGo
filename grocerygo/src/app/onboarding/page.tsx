...
export default function OnboardingPage() {
  ...
  // Scroll to top only when moving forward (not back)
  useEffect(() => {
  ...
  const handleMultipleSelect = (questionId: string, option: string, maxSelections?: number) => {
    setAnswers(prev => {
  ...
  const handleRanking = (questionId: string, options: string[], direction: 'up' | 'down', index: number) => {
                          ...
                          return (
                            <button
                              ...
                              className={`w-full rounded-xl border-2 p-4 text-left transition-all ${
                                isSelected
                                  ? 'border-[var(--gg-primary)] bg-white'
                                  : 'border-gray-200 bg-white hover:border-[var(--gg-primary)] hover:border-opacity-50'
                              }`}
                            >
